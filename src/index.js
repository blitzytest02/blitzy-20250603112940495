...
// optional environment variable, and a default for when it is absent. There is
// no config file and no `.env` to load.
//
// `process.env` is how a program reads the environment it was started in. An
// unset variable reads as `undefined`, which is falsy, so `||` falls through to
// the default — and `PORT=8080 npm start` overrides it without touching code.
...
const shutdown = () => {
  ...
  // has actually been released. Nothing calls `process.exit` afterwards: with
  // no listening socket left, there is nothing keeping the process alive, so it
  // ends on its own.
  server.close(() => {
